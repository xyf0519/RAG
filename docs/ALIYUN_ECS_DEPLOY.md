...
main 中的这套可选部署模板面向公开 Web App 上线。部署形态为 Aliyun ECS + Docker Compose + Nginx，FastAPI 与 Next.js 只在 Docker 内网通信，公网只开放 Nginx。
...
`ADMIN_EMAILS` 里的邮箱注册或登录后自动成为核心管理员。学生注册域名由 `ALLOWED_EMAIL_DOMAINS=zju.edu.cn` 控制；旧变量 `ALLOWED_EMAIL_DOMAIN` 仍可兼容单域名部署。

`.env.production.example` 默认使用 DaoCloud Docker Hub 镜像源和阿里云 PyPI 源，适合国内 ECS 网络环境。如果服务器可以稳定访问 Docker Hub 和 PyPI，可按需切回官方源。
