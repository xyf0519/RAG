...
当前 ECS 部署没有可用 NVIDIA GPU，后端使用 CPU 跑 BGE embedding。`requirements-local-models.txt` 安装的是 CPU-only torch；这不会改变当前检索效果，只是去掉当前机器用不上的 CUDA 运行库。如果未来迁移到 GPU 机器，不要直接沿用这套 CPU-only 依赖，需要重新评估镜像和 compose GPU runtime。
...
当前 ECS 的公网 IP 临时验证环境使用：

```bash
docker compose --env-file .env.public-http -f docker-compose.public-http.yml ...
```

后续命令默认沿用 `.env.public-http` + `docker-compose.public-http.yml`。如果切到严格生产模板，把命令里的 `.env.public-http` 换成 `.env.production`，把 `docker-compose.public-http.yml` 换成 `docker-compose.aliyun.yml`。
...
基础镜像由 `Dockerfile.backend.base` 构建；业务镜像由 `Dockerfile.backend` 构建。普通 Python 代码更新只需要构建 `rag-backend:latest`，不会重装 torch。
...
CPU-only torch 不改变 BGE 模型权重，不改变 embedding 维度，不改变召回参数，所以不应改变当前业务效果。它只去掉本机没有 GPU 时用不上的 CUDA wheel。
...
```json
{"ok":true,"app":"xyfRAG web","backend":{"ok":true,"app":"xyfRAG"}}
```
...
| `Dockerfile.backend` | 否 | 是 | `up -d --build backend` |
| 知识库原文 | 否 | 否 | 重建索引后重启 |
| 边界训练数据 | 否 | 否 | 重新训练后重启 |
